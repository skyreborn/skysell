					...
					<li v-show="needShow(rating.rateType,rating.text)" v-for="rating in ratings" class="rating-item border-1px">
						...
						<div class="content">
							...
							<div class="star-wrapper">
								...
								<span class="delivery-time" v-show="rating.deliveryTime">{{ rating.deliveryTime }}
								分钟送达
								</span>
							</div>
...
<script type="text/ecmascript-6">
	...
	export default {
		...
		methods: {
			...
			changeOnlyContent() {//传递给子组件ratingselect切换是否只看有内容评论的方法
				this.onlyContent = !this.onlyContent
				this.fresh()
			},
			needShow(type,text) {
				if(this.onlyContent && !text) {//判断是否只显示有内容的评论
					return false
				}
				if(this.selectType === ALL) {//选中全部直接全部显示
					return true
				}else {//推荐或吐槽
