<template>
	<div class="ratingselect">
		...
		<div @click="selectContent" class="switch" :class="{'on':onlyContent}">
	...
	export default {
		...
		methods: {
			select(type,event){
				...
				//this.selectType = type  //不建议直接修改上层组件传过来的prop值(会报错)
				this.$emit('change',type) //通过父组件向子组件传方法间接改变父组件传过来的值
			},
			selectContent(){
				if(!event._constructed) {//阻止pc端点击两次
					return
				}
				this.$emit('only') //通过父组件传方法
			}
		}
	}
